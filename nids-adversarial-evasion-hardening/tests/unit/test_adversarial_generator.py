...
class TestPGD:
    ...
    def test_pgd_stronger_than_fgsm(self):
        ...
        fgsm_adv = engine.fgsm_attack(test_data, test_labels, epsilon=0.1)
        pgd_adv = engine.pgd_attack(
            test_data, test_labels,
            epsilon=0.1, alpha=0.01, iterations=20
        )

        fgsm_diff = (fgsm_adv - test_data).abs().mean().item()
    ...
    def test_pgd_accuracy_drop(self):
        ...
        results = engine.evaluate_robustness(
            test_data, test_labels,
            lambda d, l: engine.pgd_attack(d, l, epsilon=0.3, alpha=0.05, iterations=20)
        )
