...
class AdversarialAttackEngine:
    ...
    def cw_attack(self, data, labels, targeted=False, cw_confidence=0, max_iter=100):
        ...
        for iteration in range(max_iter):
            ...
            target_prob = (probs * target_onehot).sum(dim=1)
            ...
            if iteration % 20 == 0:
                ...
                if not targeted:
                    attack_success = (preds != labels).float().mean().item()
                else:
                    attack_success = (preds == target_labels).float().mean().item()
...
def main():
    ...
    if args.attack == 'fgsm':
        results = engine.evaluate_robustness(
            ...
            lambda d, l: engine.fgsm_attack(d, l, epsilon=args.epsilon)
        )
    elif args.attack == 'pgd':
        results = engine.evaluate_robustness(
            test_data, test_labels,
            lambda d, l: engine.pgd_attack(
                d, l, epsilon=args.epsilon,
